...
class RedisClient {
    ...
    public function mset(l:Iterable<Duet<RedisKey, RedisString>>) : Promise<Noise> 
        return Promise.ofJsPromise(client.mset(new js.lib.Map([ for (dd in l) [dd.a, dd.b ] ])))
            .next( _ -> Noise );

    public function del(l:Iterable<RedisKey>) : Promise<Noise>
        return Promise.ofJsPromise(client.del(Lambda.array(l))) .next( _ -> Noise );

    public function exists(l:Iterable<RedisKey>) : Promise<Int>
        return Promise.ofJsPromise(client.exists(Lambda.array(l)));

    public function quit() : Promise<Noise>
        return Promise.ofJsPromise(client.quit()).next( _ -> Noise );
